...
def auto_delete(msg, delay=10):
    ...
    def _delete():
        ...
        try:
            ...
            print(f"auto_delete message failed: {e}")
...
def handle_start(message):
    ...
    if tid:
        try:
            ...
            print(f"/start reactivate err: {e}")
    ...
    if tid:
        try:
            ...
            print(f"/start ensure_project spec_url err: {e}")
...
def handle_forwarded_message(message):
    try:
        ...
        if orig_res.data:
            ...
            insert_res = supabase.table("casting_applications").insert([new_app]).execute()
            
            # 4. Add the Delete button to the forwarded message by sending a small "control" message
            if insert_res.data:
                markup = types.InlineKeyboardMarkup()
                new_app_id = insert_res.data[0]['id']
                markup.add(types.InlineKeyboardButton("🗑️ УДАЛИТЬ ИЗ ЭТОГО ТОПИКА", callback_data=f"app_del:{new_app_id}"))
                conf_msg = bot.reply_to(message, f"✅ Актер **{found_name}** добавлен в проект **{new_project_name}**.", reply_markup=markup)
                
                # Auto-delete confirmation message using safe executor
...
def handle_app_delete_callback(call):
    try:
        ...
        if not res.data:
            ...
            except Exception as e: print(f"app_del UI msg delete err: {e}")
            return
        ...
        if msg_id:
            ...
            except Exception as e: print(f"app_del text delete err: {e}")
            
        # Delete media group
        for mid in media_group:
            try: bot.delete_message(cid, mid)
            except Exception as e: print(f"app_del media delete err: {e}")
        ...
        if call.message.message_id != msg_id:
            try: bot.delete_message(cid, call.message.message_id)
            except Exception as e: print(f"app_del button delete err: {e}")
